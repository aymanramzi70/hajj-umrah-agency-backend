    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <h3 class="mb-4">إضافة باقة جديدة</h3>
                ...
                <form action="{{ route('packages.store') }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('type') is-invalid @enderror" id="type" name="type"
                            ...
                            <option value="Hajj" {{ old('type') == 'Hajj' ? 'selected' : '' }}>حج</option>
                            <option value="Umrah" {{ old('type') == 'Umrah' ? 'selected' : '' }}>عمرة</option>
                            <option value="Tour" {{ old('type') == 'Tour' ? 'selected' : '' }}>جولات سياحية</option>
                        </select>
                        @error('type')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="number" step="0.01"
                                ...
                                value="{{ old('agent_price_per_person') }}" min="0">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="number" class="form-control @error('number_of_days') is-invalid @enderror"
                                id="number_of_days" name="number_of_days" value="{{ old('number_of_days') }}" required
                                min="1">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('status') is-invalid @enderror" id="status"
                            ...
                            <option value="full" {{ old('status') == 'full' ? 'selected' : '' }}>ممتلئة</option>
                            <option value="archived" {{ old('status') == 'archived' ? 'selected' : '' }}>أرشيفية
                            </option>
                        </select>
                        @error('status')
                            <div class="invalid-feedback">{{ $message }}</div>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('includes_text') is-invalid @enderror" id="includes_text" name="includes_text"
                            rows="3">{{ old('includes_text') }}</textarea>
                        @error('includes_text')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('excludes_text') is-invalid @enderror" id="excludes_text" name="excludes_text"
                            rows="3">{{ old('excludes_text') }}</textarea>
                        @error('excludes_text')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
