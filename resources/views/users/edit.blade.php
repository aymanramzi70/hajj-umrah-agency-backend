    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <h3 class="mb-4">تعديل المستخدم: {{ $user->name }}</h3>
                ...
                <form action="{{ route('users.update', $user->id) }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('role') is-invalid @enderror" id="role" name="role"
                            ...
                            <option value="admin" {{ old('role', $user->role) == 'admin' ? 'selected' : '' }}>مسؤول
                                عام (Admin)</option>
                            <option value="branch_manager"
                                {{ old('role', $user->role) == 'branch_manager' ? 'selected' : '' }}>مدير فرع (Branch
                                Manager)</option>
                            <option value="reservation_agent"
                                {{ old('role', $user->role) == 'reservation_agent' ? 'selected' : '' }}>موظف حجوزات
                                (Reservation Agent)</option>
                            <option value="accountant"
                                {{ old('role', $user->role) == 'accountant' ? 'selected' : '' }}>محاسب (Accountant)
                            </option>
                            <option value="agent" {{ old('role', $user->role) == 'agent' ? 'selected' : '' }}>وكيل
                                خارجي (Agent)</option> 
                            <option value="customer_app_user"
                                {{ old('role', $user->role) == 'customer_app_user' ? 'selected' : '' }}>مستخدم تطبيق
                                عميل (Customer App User)</option>
                        </select>
                        @error('role')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="mb-3"> 
                        ...
                        <select class="form-select @error('status') is-invalid @enderror" id="status" name="status"
                            ...
                            <option value="inactive"
                                {{ old('status', $user->status) == 'inactive' ? 'selected' : '' }}>معطل</option>
                        </select>
                        @error('status')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
